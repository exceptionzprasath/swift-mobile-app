...
export function PayrollScreen({ theme }: PayrollScreenProps) {
  const { currentUser, employees, attendance, payrolls, companyConfig, requests, refreshData } = useAppContext();
            ...
            <ScrollView style={styles.payslipModalBody} contentContainerStyle={{ padding: 16 }}>
              ...
              <View style={styles.pdfTableGrid}>
                ...
                <View style={styles.pdfTableCard}>
                  ...
                  {(payrollComputation.deductions as any).advance > 0 && (
                    <View style={styles.pdfTableRow}>
                      <Text style={[styles.pdfTableLabel, { color: '#b45309', fontWeight: '600' }]}>Salary Advance Recovery</Text>
                      <Text style={[styles.pdfTableAmount, { color: '#e11d48', fontWeight: '700' }]}>-{formatInr((payrollComputation.deductions as any).advance)}</Text>
                    </View>
                  )}

                  {(payrollComputation.deductions as any).lwf > 0 && (
                    <View style={styles.pdfTableRow}>
                      <Text style={styles.pdfTableLabel}>Labour Welfare Fund (LWF)</Text>
                      <Text style={[styles.pdfTableAmount, { color: '#e11d48' }]}>-{formatInr((payrollComputation.deductions as any).lwf)}</Text>
                    </View>
                  )}

                  {((payrollComputation as any).extraDeductions || []).map((extra: any) => (
                    <View key={extra.id || extra.name} style={styles.pdfTableRow}>
                      <Text style={[styles.pdfTableLabel, { color: '#b91c1c' }]}>{extra.name || 'Other Deduction'}</Text>
                      <Text style={[styles.pdfTableAmount, { color: '#e11d48', fontWeight: '700' }]}>-{formatInr(extra.amount)}</Text>
                    </View>
                  ))}
